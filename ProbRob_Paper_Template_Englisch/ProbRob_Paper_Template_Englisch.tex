...
($0.009$\,m), the EKF is intermediate ($0.506$\,m), and the KF is the least
accurate ($0.957$\,m), confirming the theoretical ordering.
...
topic drops messages under load (the PF RMSE varied between $0.009$ and $0.69$\,m
...
The PF error stays near a few centimetres throughout, the EKF settles around
half a metre, and the KF error grows largest on the curved sections.
...
filters ($0.147$\,ms vs $\approx0.007$\,ms), yet it remains comfortably
real-time at 50\,Hz.
...
\begin{table}[!h]
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    Position RMSE [m]      & 0.957 & 0.506 & \textbf{0.009} \\
    Heading RMSE [$^\circ$]& 21.2  & 11.3  & \textbf{0.67}  \\
    Runtime / step [ms]    & 0.0065 & 0.0068 & 0.147 \\
    \bottomrule
  \end{tabular}}
...
\begin{figure*}[!t]
  ...
  \caption{EKF estimate on the S-slalom under increasing measurement-processing
           delay (0, 100 and 500\,ms): Ground Truth (black), EKF estimate
           (green dashed) with 2$\sigma$ covariance ellipses, and the known
           landmark (blue star). The annotated RMSE (averaged over 30 noise
           realisations) grows monotonically with delay, as the lagged landmark
           correction increasingly mismatches the robot's current pose.}
  \label{fig:delaytraj}
\end{figure*}
...
The PF achieved the lowest RMSE ($0.009$\,m), followed by the EKF
($0.506$\,m) and the KF ($0.957$\,m); the gap between KF and EKF is explained by
